...
module scrambler (
	input wire clk,
	input wire rst,
	bus.requester input_bus,
	bus.acknowledger output_bus
);


enum {idle,request,receiving} state_in, next_state_in;

enum {idle,request,sending} state_out, next_state_out;
...
always_comb begin // comb del output
 ...
 case (state)
	 ...
	 request: if (output_bus.ack == 1) begin
		 next_state_out = sending;
	 end else begin
		 next_state_out = state_out;
	 end
	 idle: if (state_in == receiving) begin
		 next_state_out = request;
		 output_bus.req = 1;
	 end else begin
		 output_bus.req = 0;
	 end
